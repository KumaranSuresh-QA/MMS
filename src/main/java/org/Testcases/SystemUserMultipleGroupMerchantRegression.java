	...
	private void fillSalesInfo(Map<String, String> testData, int TestcaseNo) throws Exception {
		...
		try {
			...
			if (name != null && !name.trim().isEmpty()) {
				...
				BL.enterElement(ISO.AggregatorName, name);
	...
	private String fillCompanyInfo(Map<String, String> testData, int TestcaseNo) throws Exception {
		try {
			...
			try {
				...
				if (!BL.isElementDisplayed(A.IntroPersonalInfo, "Personal info"))
				...
				CustomWebDriverManager.takeScreenshotStr(" Group Merchant : Company Info "); // Take screenshot on error
		...
		try {
			...
			try {
				...
				if (isFieldRequiredError || isInvalidFormatError) {
					...
					CustomWebDriverManager.takeScreenshotStr("Group Merchant : personal Info");
				}
		...
		try {
			...
			try {
				...
				if (isFieldRequiredError || isInvalidFormatError) {
					...
					// Take a single screenshot for any error condition
					CustomWebDriverManager.takeScreenshotStr("Group Merchant : Communication Info : Admin");
				}

			} catch (Exception e) {
				saveStatus = false;
				errorMessageBuilder.append("Unexpected Error: ").append(e.getMessage());
				CustomWebDriverManager.takeScreenshotStr("Group Merchant : Communication Admin Info");
			}
		...
		try {
			...
			try {
				...
				if (isFieldRequiredError || isInvalidFormatError) {
					...
					CustomWebDriverManager.takeScreenshotStr("Group Merchant : Communication Info : Settlement Recon");
				}
	...
	private void fillChannelConfig(Map<String, String> testData, int TestcaseNo)
		...
		try {
			...
			for (int currentRow = 0; currentRow < numberOfRows; currentRow++) {
				...
				if (!networkData.isEmpty()) {
					try {
						...
						logTestStep(TestcaseNo, "MMS : Group Merchant Onboarding : Channel Config : Network",
								networkData, Status, errorMessage);
					} catch (Exception e) {
						System.out.println(
				...
				logTestStep(TestcaseNo, "MMS : Group Merchant Onboarding : ChannelConfig : Start Date", "Valid Date",
						DateStatus, errorMessage);
				...
				logTestStep(TestcaseNo, "MMS : Group Merchant Onboarding : ChannelConfig : END Date", "Valid Date",
						DateStatus, errorMessage);
				// Process Save Button
				boolean saveStatus = true;
				StringBuilder errorMessageBuilder = new StringBuilder();
				...
				try {
					...
					if (isFieldRequiredError || isInvalidFormatError) {
						...
						// Take a single screenshot for any error condition
						CustomWebDriverManager.takeScreenshotStr("Group Merchant : Channel Config ");
					}
			...
			try {
				...
				if (!BL.isElementDisplayed(GM.IntrodiscountRate, "Disc Rate")) {
					throw new AssertionError("Assertion Error ");
				}
			} catch (AssertionError e) {

				CustomWebDriverManager.takeScreenshotStr("Group Merchant : Channel Config"); // Take screenshot on error
	...
	private void fillKYCDetails(Map<String, String> testData, int TestcaseNo)
		...
		try {
			...
			try {
				...
				CustomWebDriverManager.takeScreenshotStr("Group Merchant : KYC");
			}
			...
			// Handle and log exceptions
			ExceptionHandler exceptionHandler = new ExceptionHandler(driver, ExtentCucumberAdapter.getCurrentStep());
			exceptionHandler.handleException(e, "Group Merchant : KYC");
			throw e;
		}
	}
	...
	private void FillDiscountRate(Map<String, String> testData, int TestcaseNo)
		...
		try {
			...
			for (int currentRow = 0; currentRow < numberOfRows; currentRow++) {
				...
				try {
					...
					if (isFieldRequiredError || isInvalidFormatError) {
						...
						// Take a single screenshot for any error condition
						CustomWebDriverManager.takeScreenshotStr("Group Merchant : Discount");
					}

				} catch (Exception e) {
					saveStatus = false;
					errorMessageBuilder.append("Unexpected Error: ").append(e.getMessage());
					CustomWebDriverManager.takeScreenshotStr("Group Merchat :  Discount Rate");
				}
			...
			try {
				...
				CustomWebDriverManager.takeScreenshotStr("Group Merchant : Discount Rate "); // Take screenshot on error
		...
		try {
			...
			try {
				...
				if (isFieldRequiredError || isInvalidFormatError) {
					...
					// Take a single screenshot for any error condition
					CustomWebDriverManager.takeScreenshotStr("Group Merchant : Settlement Info");
				}
		...
		try {
			...
			try {
				...
				if (isFieldRequiredError || isInvalidFormatError) {
					...
					// Take a single screenshot for any error condition
					CustomWebDriverManager.takeScreenshotStr("Group Merchant : Whitelabel");
				}
			...
			logTestStep(TestcaseNo, "MMS : Group Merchant Onboarding : Save Button", "Whitelabel", saveStatus,
					errorMessage);
		...
		try {
			...
			try {
				...
				if (isFieldRequiredError || isInvalidFormatError) {
					...
					// Take a single screenshot for any error condition
					CustomWebDriverManager.takeScreenshotStr("Group Merchant : Webhooks ");
				}
